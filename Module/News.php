...
if($__TOKEN = "CopyrightByRefleXx"){
	$conn = sqlsrv_connect($__CONFIG['SQLHost'], array("Database"=>$__CONFIG['SQLDB'], "UID"=>$__CONFIG['SQLUID'], "PWD"=>$__CONFIG['SQLPWD']));
				...
				<?php
		$getContent = sqlsrv_query($conn, "SELECT TOP 50 title, text, author, date FROM Account..tWebNews ORDER BY date DESC;", array(), array("Scrollable" => SQLSRV_CURSOR_KEYSET));
